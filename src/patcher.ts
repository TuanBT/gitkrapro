...
export class Patcher {
  ...
  public backupAsar(): string {
    ...
    if (!fs.existsSync(backup)) {
      throw new Error(`Backup creation failed: ${backup}`);
    }
    return backup;
  }

  /**
   * Restore app.asar from backup file
   * @param backup Path to backup file
   */
  public restoreBackup(backup: string): void {
    if (fs.existsSync(backup)) {
      fs.copySync(backup, this.asar);
    }
  }
  ...
  private patchDirWithFeature(feature: string): void {
    const patchFile = path.join(baseDir, "patches", `${feature}.diff`);

...
async function executeActions(actions) {
    ...
    try {
        for (const action of actions) {
            switch (action) {
                case 1: {
                    backupPath = patcher.backupAsar();
                    console.log(`${chalk_1.default.green("==>")} ${emoji.get("package")} Backup ` +
                        `${chalk_1.default.green(patcher.asar)} ➔ ${chalk_1.default.green(backupPath)}`);
                    break;
                }
                case 2: {
        ...
        console.error(`${chalk_1.default.red("==>")} ${emoji.get("x")} Error: ${error.message}`);
        if (backupPath) {
            console.log(`${chalk_1.default.yellow("==>")} Restoring backup from ${chalk_1.default.green(backupPath)}...`);
            patcher.restoreBackup(backupPath);
        }
        try {
            patcher.removeDir();
        }
...
program
    .name("gitkrapro-patcher")
    .description("GitKraPro patcher")

...
async function executeActions(actions: Actions[]) {
  ...
  try {
        ...
        case Actions.backup: {
          ...
          console.log(
            `${chalk.green("==>")} ${emoji.get("package")} Backup ` +
              `${chalk.green(patcher.asar)} ➔ ${chalk.green(backupPath)}`,
          );
          break;
        }
        case Actions.unpack: {
    ...
    console.error(`${chalk.red("==>")} ${emoji.get("x")} Error: ${(error as Error).message}`);
    if (backupPath) {
      console.log(`${chalk.yellow("==>")} Restoring backup from ${chalk.green(backupPath)}...`);
      patcher.restoreBackup(backupPath);
    }
    try { patcher.removeDir(); } catch { /* cleanup */ }
    process.exit(1);
  }
}
